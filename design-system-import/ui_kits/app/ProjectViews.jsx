      ...
      <div draggable onDragStart={onDragStart} onDragEnd={onDragEnd}
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <span style={{ display: "flex", alignItems: "center", gap: 7 }}>
            <DBadge tone={PRIORITY_TONE[task.priority]}>{task.priority}</DBadge>
          ...
          <DAvatar name={task.assignee} size="xs" />
        </div>
      </div>
    </Tip>
  );
...
function BoardView({ f, onOpenTask, onApplyGroup, onAdd }) {
  ...
  const map = {}; f.filtered.forEach((t) => { const k = meta.key(t); (map[k] = map[k] || []).push(t); });
      ...
      {cols.map((col) => {
        ...
        return (
          <div key={col}
            ...
            <QuickAdd onAdd={(title) => onAdd({ title, [meta.field]: col })} />
          </div>
        );
      })}
    </div>
...
/* ============================ List view ============================ */
function ListRow({ task, done, onToggle, onOpen }) {
  const [hover, setHover] = React.useState(false);
  return (
...
function ListView({ f, completed, onToggle, onOpenTask, onAdd }) {
      ...
      {groups.map(([label, tasks]) => (
        <div key={label}>
          ...
          <div style={{ borderTop: "1px solid var(--border)", padding: "4px 12px" }}>
            <QuickAdd onAdd={(title) => onAdd({ title, [meta.field]: f.groupBy === "none" ? undefined : label })} />
          </div>
        </div>
      ))}
    </div>
...
function MonthCell({ date, tasks, isToday, isOver, onOpen, onDrop, onOver, onLeave, adding, setAdding, onAdd }) {
  return (
    <div
...
function DayColumn({ date, tasks, isToday, isOver, onOpen, onDrop, onOver, onLeave, adding, setAdding, onAdd, flex }) {
  return (
...
function CalendarView({ f, onOpenTask, onSchedule, onAdd }) {
...
/* ============================ Timeline view ============================ */
function TimelineView({ f, onOpenTask, onReschedule, onResize, onAdd }) {
  const dayW = 60, labelW = 232, rowH = 46;
  const days = React.useMemo(() => Array.from({ length: TL_DAYS }, (_, i) => idxToDue(i)), []);
  const isWeekend = (i) => { const d = days[i].getDay(); return d === 0 || d === 6; };
  ...
  const onMove = (e) => {
    ...
    if (d.type === "move") onReschedule(d.id, Math.max(0, Math.min(TL_DAYS - d.len, d.orig + delta)));
    else onResize(d.id, Math.max(1, Math.min(TL_DAYS - d.start, d.origLen + delta)));
  };
  const onUp = () => {
  ...
  const startMove = (e, t) => { e.preventDefault(); drag.current = { type: "move", id: t.id, task: t, startX: e.clientX, orig: t.start, len: t.len, moved: false }; window.addEventListener("pointermove", onMove); window.addEventListener("pointerup", onUp); };
  const startResize = (e, t) => { e.preventDefault(); e.stopPropagation(); drag.current = { type: "resize", id: t.id, startX: e.clientX, start: t.start, origLen: t.len }; window.addEventListener("pointermove", onMove); window.addEventListener("pointerup", onUp); };
                ...
                {tasks.map((t) => (
                  <div key={t.id} style={{ display: "flex", alignItems: "center", height: rowH, borderTop: "1px solid var(--border)" }}>
                    ...
                    <div style={{ position: "relative", flex: 1, height: "100%" }}>
                      {days.map((d, idx) => <div key={idx} style={{ position: "absolute", top: 0, bottom: 0, left: idx * dayW, width: dayW, borderLeft: "1px solid var(--border)", background: isWeekend(idx) ? "var(--bg-app)" : "transparent" }} />)}
